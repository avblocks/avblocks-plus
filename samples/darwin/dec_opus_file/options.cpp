...
{
    opt.inputFile = getExeDir() + "/../../assets/aud/Everybody-TBB.opus";

    fs::path output(getExeDir() + "/../../output/dec_opus_file");
    fs::create_directories(output);

    ostringstream s;
    s << output.c_str() << "/Everybody-TBB.wav";
    opt.outputFile = s.str();
}

void help(OptionsConfig<char>& optcfg)
{
    cout << "dec_opus_file --input <opus file> --output <wav file>" << endl;
...
ErrorCodes prepareOptions(Options &opt, int argc, char* argv[])
{
    ...
    optcfg.addOptions()
    ("help,h", opt.help, "")
    ("input,i", opt.inputFile, string(), "input Opus file")
