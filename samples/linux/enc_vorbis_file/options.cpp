...
void setDefaultOptions(Options& opt)
{
    ...
    fs::path output(getExeDir() + "/../../output/enc_vorbis_file");
    fs::create_directories(output);

    ostringstream s;
    s << output.c_str() << "/equinox-48KHz.ogg";
    opt.outputFile = s.str();
}

void help(OptionsConfig<char>& optcfg)
{
    cout << "enc_vorbis_file --input <wav file> --output <ogg file>" << endl;
...
ErrorCodes prepareOptions(Options &opt, int argc, char* argv[])
{
    ...
    ("output,o", opt.outputFile, string(), "output AAC ADTS file");
