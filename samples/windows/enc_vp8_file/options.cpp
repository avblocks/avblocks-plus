...
void setDefaultOptions(Options& opt)
{
    ...
    std::filesystem::path output(getExeDir() + L"/../../output/enc_vp8_file");
...
{
    wcout << L"enc_vp8_file --frame <width>x<height> --rate <fps> --color <COLOR> --input <file.yuv> --output <file.ivf>" << endl
          << L"[--colors]" << endl;
    doHelp(wcout, optcfg);
}
...
ErrorCodes prepareOptions(Options& opt, int argc, wchar_t* argv[])
{
    if (argc < 2)
    {
        ...
        wcout << L"enc_vp8_file --input " << opt.yuv_file;
