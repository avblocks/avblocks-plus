...
void setDefaultOptions(Options& opt)
{
    ...
    fs::path output(getExeDir() + L"/../../output/enc_avc_pull");
...
{
    wcout << L"enc_avc_pull --frame <width>x<height> --rate <fps> --color <COLOR> "
             L"--input <file.yuv> --output <file.h264> [--colors]\n";
...
bool validateOptions(Options& opt)
{
    ...
    if (opt.h264_file.empty())  { wcout << L"output file needed\n"; return false; }
...
ErrorCodes prepareOptions(Options& opt, int argc, wchar_t* argv[])
{
    ...
    optcfg.addOptions()
        ...
        (L"output,o", opt.h264_file,      wstring(),          L"output H.264 file")
