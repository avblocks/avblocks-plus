...
void setDefaultOptions(Options& opt)
{
    ...
    fs::path output(getExeDir() + L"/../../output/enc_g711_alaw_file");
    fs::create_directories(output);

    wostringstream s;
    s << output.c_str() << L"/express-dictate_g711_alaw.wav";
    opt.outputFile = s.str();
}

void help(OptionsConfig<wchar_t>& optcfg)
{
    wcout << L"enc_g711_alaw_file --input <wav file> --output <g711 alaw wav file>" << endl;
