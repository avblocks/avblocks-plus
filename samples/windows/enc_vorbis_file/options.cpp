...
void setDefaultOptions(Options& opt)
{
    ...
    std::filesystem::path output(getExeDir() + L"/../../output/enc_vorbis_file");
    std::filesystem::create_directories(output);

    wostringstream s;
    s << output.c_str() << L"/equinox-48KHz.ogg";
    opt.outputFile = s.str();
}

void help(OptionsConfig<wchar_t>& optcfg)
{
    wcout << L"enc_vorbis_file --input <wav file> --output <ogg file>" << endl;
...
ErrorCodes prepareOptions(Options& opt, int argc, wchar_t* argv[])
{
    ...
    optcfg.addOptions()
        ...
        (L"output,o", opt.outputFile, wstring(), L"output OGG file");
