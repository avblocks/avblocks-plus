...
{
    opt.inputFile = getExeDir() + L"/../../assets/aud/Hydrate-Kenny_Beltrey.mp3";

    fs::path output(getExeDir() + L"/../../output/audio_upsample");
    fs::create_directories(output);

    wostringstream s;
    s << output.c_str() << L"/Hydrate-Kenny_Beltrey-48khz.mp3";
    opt.outputFile = s.str();
}

void help(OptionsConfig<wchar_t>& optcfg)
{
    wcout << L"audio_upsample --input <mp3 file> --output <mp3 file>" << endl;
...
ErrorCodes prepareOptions(Options &opt, int argc, wchar_t* argv[])
{
    ...
    optcfg.addOptions()
    (L"help,h", opt.help, L"")
    (L"input,i", opt.inputFile, wstring(), L"input MP3 file")
