...
{
    opt.inputFile = getExeDir() + L"/../../assets/aud/express-dictate_8000_s16_1ch_pcm.wav";

    fs::path output(getExeDir() + L"/../../output/enc_g711_ulaw_file");
    fs::create_directories(output);

    wostringstream s;
    s << output.c_str() << L"/express-dictate_g711_ulaw.wav";
    opt.outputFile = s.str();
}

void help(OptionsConfig<wchar_t>& optcfg)
{
    wcout << L"enc_g711_ulaw_file --input <wav file> --output <g711 ulaw wav file>" << endl;
...
ErrorCodes prepareOptions(Options &opt, int argc, wchar_t* argv[])
{
    ...
    optcfg.addOptions()
        ...
        (L"output,o", opt.outputFile, wstring(),        L"output G.711 U-law WAV file");
