...
void setDefaultOptions(Options& opt)
{
    ...
    fs::path output(getExeDir() + L"/../../output/enc_aac_adts_file");
    fs::create_directories(output);

    wostringstream s;
    s << output.c_str() << L"/equinox-48KHz.adts.aac";
    opt.outputFile = s.str();
}

void help(OptionsConfig<wchar_t>& optcfg)
{
    wcout << L"enc_aac_adts_file --input <wav file> --output <aac file>" << endl;
...
ErrorCodes prepareOptions(Options &opt, int argc, wchar_t* argv[])
{
    ...
    (L"output,o", opt.outputFile, wstring(), L"output AAC ADTS file");
